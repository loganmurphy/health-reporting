...
import { loadDevVars, saveDevVars, openBrowser, copyToClipboard } from "../../scripts/utils"
...
describe("openBrowser", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks()
    mockSpawnSync.mockReturnValue({ status: 0, pid: 1, output: [], stderr: Buffer.from(""), stdout: Buffer.from(""), signal: null })
  })
  ...
  it("uses 'cmd /c start' on Windows", () => {
    ...
    openBrowser("https://example.com")
    expect(mockSpawnSync).toHaveBeenCalledWith("cmd", ["/c", "start", "https://example.com"], { stdio: "ignore" })
  })

  it("uses 'xdg-open' on Linux", () => {
    Object.defineProperty(process, "platform", { value: "linux" })
    openBrowser("https://example.com")
    expect(mockSpawnSync).toHaveBeenCalledWith("xdg-open", ["https://example.com"], { stdio: "ignore" })
  })
})
...
describe("copyToClipboard", () => {
  ...
  it("uses 'pbcopy' on macOS and returns true on success", () => {
    ...
    expect(mockSpawnSync).toHaveBeenCalledWith("pbcopy", [], { input: "hello", stdio: ["pipe", "ignore", "ignore"] })
  ...
  it("uses 'clip' on Windows", () => {
    ...
    expect(mockSpawnSync).toHaveBeenCalledWith("clip", [], { input: "hello", stdio: ["pipe", "ignore", "ignore"] })
  })
  ...
  it("uses 'xclip -selection clipboard' on Linux", () => {
    ...
    mockSpawnSync.mockReturnValue({ status: 0, pid: 1, output: [], stderr: Buffer.from(""), stdout: Buffer.from(""), signal: null })
    copyToClipboard("hello")
    expect(mockSpawnSync).toHaveBeenCalledWith("xclip", ["-selection", "clipboard"], { input: "hello", stdio: ["pipe", "ignore", "ignore"] })
  })

  it("returns false when command fails", () => {
    Object.defineProperty(process, "platform", { value: "darwin" })
    mockSpawnSync.mockReturnValue({ status: 1, pid: 1, output: [], stderr: Buffer.from(""), stdout: Buffer.from(""), signal: null })
    const result = copyToClipboard("hello")
